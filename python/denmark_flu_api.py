...
for age_grp in age_groups:
    print(f"Querying influenza data for age group: {age_grp}")
    for value_type in ["cases", "tested", "admissions", "deaths", "incidens"]:
        q = urllib.parse.quote(f"(agegrp='{age_grp}')", safe="()")
        s = (
            "https://services5.arcgis.com/Hx7l9qUpAnKPyvNz/arcgis/rest/services/DB_Influenza_data_gdb/FeatureServer//query?f=json&cacheHint=true&groupByFieldsForStatistics=week_sort%2CSeason&orderByFields=week_sort%20ASC&outFields=*&outStatistics=%5B%7B%22onStatisticField%22%3A%22"
            + value_type
        ...
        for item in content["features"]:
            ...
            year_week = week_sort_raw.replace("Ã…r", "Year").replace("uge", "week").strip()
...
# Save to CSV
df.to_csv("csv/Denmark/influenza_denmark.csv")
